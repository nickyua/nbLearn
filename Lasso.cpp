...
void Lasso::fit(Matrix &_X_train, vector<double> &_yTrain, double _l1, double _tolerance, double _step_size)
{
	assert(_X_train.size() == _yTrain.size() && "LinearRegression::fit: different dimension sizes");

	this->gradientDescent(_X_train, _yTrain, _l1, _tolerance, _step_size);
}

vector<double> Lasso::featureDerivative(vector<double>& _errors, Matrix &_features) const {
...
vector<double> Lasso::predict(Matrix &_X_test) const
{
...
void Lasso::gradientDescent(Matrix &_X_train, vector<double> &_yTrain, double _l1, double _tolerance, double _step_size)
{
	//	_tolerance = 0.01;
	...
	while (!converged /*&& amountSteps < 1e6*/)
	{
		...
		m_aWeights = (1.0 - 2 * _l1 * _step_size) * m_aWeights - _step_size*derivative;
		m_aWeights[0] += _step_size*derivative[0];
		m_aWeights[0] /= (1.0 - 2 * _l1 * _step_size);
		m_aWeights[0] -= _step_size * derivative[0];
		if (norm < _tolerance)
		{
			converged = true;
...
double Lasso::evaluate(Matrix &_X_test, vector<double> &_y_test, double(*_evaluation_metric)(const vector<double>&y, const vector<double>&prediction)) const {
